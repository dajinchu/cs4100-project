...
INIT_EXPLORE_PROB = 0.2
END_EXPLORE_PROB = 0.2
    ...
    for i_episode in range(ITERATIONS):
        ...
        for t in range(MAX_MOVES):
            ...
            if enables == [SKIP_ACTION]:
                action = SKIP_ACTION
                # observation is the state generated by this action
                observation, reward, done, info = env.step(action)
            else:
                # this evaluation is the Q-table generated by the network for this state
                evaluation = Q(observation)

                # mask the invalid moves by setting them to 0, then select the maximum q-value
                mask = valid_mask(enables)
                max_q = torch.max(evaluation * mask)

                # select an action, either by selecting the argmax of the q-values or by selecting a random value
